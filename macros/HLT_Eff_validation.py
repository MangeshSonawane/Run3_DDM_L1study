...
#                if (qual < 11): continue
		...
		for j in EmuMus_unique:
			...
			if dR_corr < 0.4 : dR_list_corr.append([dR_corr, i, j, mutype])
...
############################################################
		...
		h_pt_gen.Fill(float(Gen_br.partPt[genmu_idx]))
...
#####################
    ...
    for i in range(3) :
	for j in range(3) :
		UPTres_EMTF[i][j] = UPTres_EMTF[i][j]/UPTres_EMTF_count[i][j]
		PTres_EMTF[i][j] = PTres_EMTF[i][j]/PTres_EMTF_count[i][j]
